...
SLIPEncodedStream::SLIPEncodedStream(Stream &s)
{
	serial = &s;
	rstate = CHAR;
}
...
/*
 SERIAL METHODS
 */
bool SLIPEncodedStream::endofPacket()
{
	if (rstate == SECONDEOT)
	{
		rstate = CHAR;
...
int SLIPEncodedStream::available()
{
back:
...
//encode SLIP
size_t SLIPEncodedStream::write(uint8_t b)
{
	if (b == eot)
	{
...
size_t SLIPEncodedStream::write(const uint8_t *buffer, size_t size)
{
	size_t result = 0;
	while (size--)
		result = write(*buffer++);
...
void SLIPEncodedStream::beginPacket() { serial->write(eot); }

//signify the end of the packet with an EOT
void SLIPEncodedStream::endPacket()
{
	serial->write(eot);
}

void SLIPEncodedStream::flush()
{
	serial->flush();
}
